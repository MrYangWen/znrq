package com.cq.xm.znrq.view.customdialog;
 ...
 * 对app的所有对话框进行管理
 * <p>
 * Created by luojiang on 2015/9/21.
 */
...
public class DialogMaker {
     ...
     * 创建一个通用的alert对话框
     *
     * @param title                  对话框的标题。为null或""没有标题
     * @param msg                    对话框要显示内容
     * @param btns                   显示按钮的标题。为null没有按钮
    ...
    public static Dialog showCommonAlertDialog(Context context, String title, String msg, String[] btns, final DialogCallBack callBack, boolean isCanCancelabel, final boolean isDismissAfterClickBtn, final Object tag) {
        ...
        View contentView = LayoutInflater.from(context).inflate(R.layout.frame_dialog_alert_common_layout, null);
        ...
        if (null != btns) {
            ...
            if (len > 0) {
                ...
                Button btn;
                View LineView;
                LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(0, ViewUtil.dip2px(context, 44));
                params.weight = 1.0f;
                for (int i = 0; i < len; i++) {
                    ...
                    btn.setTextSize(TypedValue.COMPLEX_UNIT_PX, ViewUtil.dip2px(context, 16));
                    btn.setTextColor(0xff007cff);
                    btn.setGravity(Gravity.CENTER);
                    if (0 == i && 1 == len) {
                        btn.setBackgroundResource(R.drawable.frame_alert_single_btn_selector);
                    } else if (0 == i) {
                        btn.setBackgroundResource(R.drawable.frame_alert_left_btn_selector);
                    } else if (i > 0 && len > 2) {
                        btn.setBackgroundResource(R.drawable.frame_alert_mid_btn_selector);
                    } else if (len - 1 == i) {
                        btn.setBackgroundResource(R.drawable.frame_alert_right_btn_selector);
                    }
        ...
        } else if (StrUtil.isNotBlank(msg)) {
        ...
        if (StrUtil.isNotBlank(msg)) {
            final int margin = ViewUtil.dip2px(context, 21.33f);
            ...
            final int margin = ViewUtil.dip2px(context, 38.67f);
            mParams.topMargin = margin;
        ...
        aWmLp.width = ScreenUtil.defaultCenter().getWidth() - 100;
    ...
    public static void setDialogTextViewContent(Context context, String content, TextView tView) {
        ...
        if (content.contains(NEW_LINE) || content.contains("\n")) {
            ...
            float destWidth = StrUtil.getContentWidth(content, tView);
            float maxWidth = ViewUtil.dip2px(context, 235.33f);
    ...
    public static Dialog showCommenWaitDialog(Context context, String msg, final DialogCallBack callBack, boolean isCanCancelabel, final Object tag) {
        ...
        View contentView = LayoutInflater.from(context).inflate(R.layout.frame_dialog_wait_common_layout, null);
        TextView contentTv = (TextView) contentView.findViewById(R.id.dialog_content_tv);

//        ImageView mIvTest = (ImageView) contentView.findViewById(R.id.iv_loading);
//        AnimationDrawable anim = (AnimationDrawable) mIvTest.getBackground();
//        anim.start();

//        Rotate3dAnimation rotation = new Rotate3dAnimation(0, 360, ViewUtil.dip2px(context, 35), ViewUtil.dip2px(context, 35),
//                0, false);
//        // 动画持续时间500毫秒
...
//
//        mIvTest.startAnimation(rotation);
